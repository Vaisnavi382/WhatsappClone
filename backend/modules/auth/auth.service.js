...
export const signupUser = async (req, res) => {
  try {
    ...
    if (!isFieldValid)
      throw new CustomError(400, STATUS_MESSAGES.FIELD_VALIDATION_ERROR);

    const isUser = await user.findOne({
      $or: [{ email: email }, { phonenumber: phonenumber }],
    });
    if (isUser) throw new CustomError(400, STATUS_MESSAGES.USER_ALREADY_EXISTS);
...
export const loginUser = async (req, res) => {
  try {
    ...
    if (!isUser)
      throw new CustomError(400, STATUS_MESSAGES.INVALID_CREDENTIALS);
